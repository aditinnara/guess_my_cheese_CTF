...
def main():
    ...
    salt_for_cheese = ''.join(random.choice(characters) for _ in range(3)).lower()

    # choose and encrypt the cheese we want player to guess
    actual_cheese = random.choice(list_of_cheeses).lower() + salt_for_cheese.lower()
    ...
    print("Hint: The cheeses are top secret and limited edition, so they might look different from cheeses you're used to!")


    for i in range(3):
        print("Commands: (g)uess my cheese or (e)ncrypt a cheese")
        ...
        if choice == 'g':
            ...
            if cheese_to_encrypt in list_of_cheeses:
                your_encrypted_cheese = affine_encrypt(cheese_to_encrypt, a, b)
                print("Here's your encrypted cheese: ", your_encrypted_cheese)
